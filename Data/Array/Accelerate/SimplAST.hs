module Data.Array.Accelerate.SimpAST (
...
-- de Bruijn indices
-- -----------------------
-- We'll go ahead and use de Bruijn indicies, but why do we need types?

data Idx env where
  ZeroIdx :: Idx env
  SuccIdx :: Idx env -> Idx env
...
--
-- I'm not a big fan of all this extra whitespace
prj :: Idx env -> Val env -> t
...
-- If this is necessary, why not use some integer representation in the
-- first place?
